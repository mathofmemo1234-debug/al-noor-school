...
export default function WeeklyPlanView({ studentClass = null, schoolId }) {
  ...
  const DAYS = [t('days.sunday'), t('days.monday'), t('days.tuesday'), t('days.wednesday'), t('days.thursday')];
  const PERIODS = [1, 2, 3, 4, 5, 6, 7, 8];
  const WEEKS = Array.from({length: 18}, (_, i) => `${t('weeklyPlan.week')} ${i + 1}`);
  ...
  useEffect(() => {
    ...
    const targetSchoolId = schoolId || 'default_school_1';
    const q = query(collection(db, 'preparations'), where('schoolId', '==', targetSchoolId));
  ...
  return (
    <div className="glass-panel" style={{ padding: '24px' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
        <div>
          <h2>{t('weeklyPlan.title')} {studentClass ? `- ${t('weeklyPlan.classWord')} ${studentClass}` : ''}</h2>
          <p style={{ color: 'var(--color-text-muted)' }}>{t('weeklyPlan.subtitle')}</p>
        </div>
        
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', background: 'rgba(255,255,255,0.5)', padding: '16px', borderRadius: '12px' }}>
          <div style={{ flex: '1 1 200px' }}>
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold', fontSize: '0.9em' }}>{t('weeklyPlan.academicYear')}</label>
          ...
          <div style={{ flex: '1 1 200px' }}>
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold', fontSize: '0.9em' }}>{t('weeklyPlan.semester')}</label>
          ...
          {!studentClass && (
            <div style={{ flex: '1 1 200px' }}>
              <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold', fontSize: '0.9em' }}>{t('weeklyPlan.classLabel')}</label>
          ...
          <div style={{ flex: '1 1 200px' }}>
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold', fontSize: '0.9em' }}>{t('weeklyPlan.weekLabel')}</label>
      ...
      {Object.keys(scheduleData).length === 0 ? (
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="data-table">
            <thead>
              <tr>
                <th style={{ width: '80px' }}>{t('weeklyPlan.day')}</th>
                {PERIODS.map(p => <th key={p} style={{ textAlign: 'center', minWidth: '150px' }}>{t('weeklyPlan.period')} {p}</th>)}
              </tr>
            </thead>
            <tbody>
              {DAYS.map(day => (
                      ...
                      cellContent = (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', background: 'rgba(99,178,198,0.05)', padding: '12px', borderRadius: '8px', textAlign: 'right', border: '1px solid rgba(99,178,198,0.2)' }}>
                          <div style={{ borderBottom: '1px solid rgba(0,0,0,0.05)', paddingBottom: '8px', marginBottom: '4px' }}>
                            ...
                            <span style={{ fontSize: '11px', color: 'var(--color-text-muted)' }}>{t('weeklyPlan.teacherPrefix')} {teachers[cell.teacherId]?.name || t('weeklyPlan.notSpecified')}</span>
                          ...
                          <div style={{ fontSize: '13px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            <div>
                              <strong style={{ color: '#334155' }}>{t('weeklyPlan.goals')}</strong> 
                              <span style={{color: '#475569'}}> {dayPlan.goals ? dayPlan.goals.substring(0, 50) + (dayPlan.goals.length > 50 ? '...' : '') : t('weeklyPlan.notSpecified')}</span>
                            </div>
                            <div>
                              <strong style={{ color: '#334155' }}>{t('weeklyPlan.content')}</strong> 
                              <span style={{color: '#475569'}}> {dayPlan.content ? dayPlan.content.substring(0, 50) + (dayPlan.content.length > 50 ? '...' : '') : t('weeklyPlan.notSpecified')}</span>
                            </div>
                          </div>
                        </div>
                      );
